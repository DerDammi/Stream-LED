...
function createApiRouter(effectManager, twitch) {
  ...
  router.post('/lamps/:id/test', express.json(), async (req, res) => { const { action, color, effect_name, effect_speed, effect_intensity, segment_mode, segment_ids, segment_colors, duration_ms } = req.body; const durationMs = Math.max(1000, Number(duration_ms || 10000)); const segmentOptions = { durationMs, segment_mode: segment_mode === 'selected' ? 'selected' : 'all', segment_ids: Array.isArray(segment_ids) ? segment_ids : [], segment_colors: sanitizeSegmentColors(segment_colors, '#9147ff') }; const effectiveColor = segmentOptions.segment_mode === 'selected' && segmentOptions.segment_colors.length ? segmentOptions.segment_colors[0].color : (color || '#ffffff'); const ok = action === 'off' ? await effectManager.setLampOff(req.params.id, segmentOptions) : action === 'effect' ? await effectManager.setLampEffect(req.params.id, effect_name, { speed: effect_speed, intensity: effect_intensity, primaryColor: effectiveColor, ...segmentOptions }) : await effectManager.setLampColor(req.params.id, effectiveColor, segmentOptions); res.json({ success: ok, duration_ms: durationMs }); });
